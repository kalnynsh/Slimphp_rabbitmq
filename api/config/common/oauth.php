...
return [
    Server\AuthorizationServer::class =>
        function (ContainerInterface $container) {
            ...
            $server->enableGrantType(
                new Server\Grant\ImplicitGrant(new \DateInterval('PT1H'))
            );

            $grant = new Server\Grant\PasswordGrant(
                $userRepository,
                $refreshTokenRepository
            );
        ...
        Server\Repositories\RefreshTokenRepositoryInterface::class =>
        function (ContainerInterface $container) {
    ...
    'config' => [
        'oauth' => [
            'public_key_path' =>
                dirname(__DIR__, 2) . '/' . getenv('API_OAUTH_PUBLIC_KEY_PATH'),
            'private_key_path' =>
